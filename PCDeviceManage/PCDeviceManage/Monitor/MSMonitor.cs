	...
	public class MSMonitor
	{
		...
		public static IEnumerable<DesktopItem> EnumerateDesktopMonitors()
		{
			...
			using (var @class = new ManagementClass("Win32_DesktopMonitor"))
			{
				...
				{
					//Debug.WriteLine($"Failed to get and enumerate instances by Win32_DesktopMonitor. HResult: {me.HResult} ErrorCode: {me.ErrorCode}" + Environment.NewLine	+ me);
					yield break;
				}
			}
			...
			using (var @class = new ManagementClass(@"root\wmi", "WmiMonitorBrightness", null))
			{
				...
				{
					//Debug.WriteLine($"Failed to get instances by WmiMonitorBrightness. HResult: {me.HResult} ErrorCode: {me.ErrorCode}" + Environment.NewLine + me);
					yield break;
				}
				...
				using (var enumerator = instances.GetEnumerator())
				{
					while (true)
					{
						...
						catch (ManagementException me)
						{
							...
							//Debug.WriteLine($"Failed to enumerate instances by WmiMonitorBrightness. HResult: {me.HResult} ErrorCode: {me.ErrorCode}" + Environment.NewLine	+ me);
							yield break;
						}
		...
		public static int GetBrightness(string deviceInstanceId)
		{
			...
			{
				//Debug.WriteLine($"Failed to get brightness by WmiMonitorBrightness. HResult: {me.HResult} ErrorCode: {me.ErrorCode}" + Environment.NewLine + me);
				return -1;
			}
		}
		...
		public static bool SetBrightness(string deviceInstanceId, int brightness, int timeout = int.MaxValue)
		{
			...
			{
				//Debug.WriteLine($"Failed to set brightness by WmiSetBrightness. HResult: {me.HResult} ErrorCode: {me.ErrorCode}" + Environment.NewLine	+ me);
				return false;
			}
		}
		...
		public static (ManagementEventWatcher watcher, string message) StartBrightnessEventWatcher()
		{
			...
			catch (ManagementException me)
			{
				...
				//Debug.WriteLine(message + Environment.NewLine + me);
