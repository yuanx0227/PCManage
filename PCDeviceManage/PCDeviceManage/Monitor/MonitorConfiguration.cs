   ...
   public class MonitorConfiguration
    {
		...
		private static MonitorCapability GetMonitorCapability(SafePhysicalMonitorHandle physicalMonitorHandle, bool verbose)
		{
			...
			return new MonitorCapability(
			 ...
			 string MakeCapabilitiesReport(byte[] vcpCodes)
			{
			 ...
			 byte[] GetCapabilitiesData(SafePhysicalMonitorHandle physicalMonitorHandles, uint capabilitiesStringLengths)
			{
		...
		private static IEnumerable<byte> EnumerateVcpCodes(string source)
		{
			...
			//static 
			bool IsAscii(char c) => c <= 0x7F;
			bool IsHexNumber(char c) =>  (c>= '0' &&  c<= '9') || (c>= 'A' && c<= 'F') || (c>= 'a' && c<= 'f');
		}
		...
		private static AccessStatus GetStatus(int errorCode)
		{
            ...
            switch (unchecked((uint)errorCode))
            {
				...
				case ERROR_GRAPHICS_I2C_ERROR_TRANSMITTING_DATA:
					return AccessStatus.TransmissionFailed;
	...
	public class MonitorCapability
	{
			...
			byte[] capabilitiesData = null)
		{
			...
			this.CapabilitiesData = (capabilitiesData != null) ? Convert.ToBase64String(capabilitiesData) : null;
		}
